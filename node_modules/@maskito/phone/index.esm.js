...
import { AsYouType, parsePhoneNumber, formatIncompletePhoneNumber, getCountryCallingCode, validatePhoneNumberLength } from 'libphonenumber-js/core';
...
function convertToNationalFormat(value, countryIsoCode, metadata) {
    ...
    try {
        ...
        return formatIncompletePhoneNumber(phone.nationalNumber, countryIsoCode, metadata);
    }
    catch {
        return '';
    }
            ...
            return {
                elementState: {
                    value: format === 'NATIONAL'
                        ? formatIncompletePhoneNumber(nationalNumber, countryIsoCode, metadata)
                        : `${code} ${nationalNumber}`,
                    selection,
                },
            };
...
function maskitoPhoneNonStrict({ defaultIsoCode, metadata, separator = '-', }) {
...
function maskitoPhoneStrict({ countryIsoCode, metadata, separator = '-', format = 'INTERNATIONAL', }) {
    const isNational = format === 'NATIONAL';
    const code = getCountryCallingCode(countryIsoCode, metadata);
    const formatter = new AsYouType(countryIsoCode, metadata);
...
function maskitoPhone({ countryIsoCode, metadata, strict = true, separator = '-', format = 'INTERNATIONAL', }) {
...
export { maskitoGetCountryFromNumber, maskitoPhone, maskitoPhone as maskitoPhoneOptionsGenerator };
