...
class NgCliFileSystem {
    ...
    globSync(patterns) {
        ...
        return allFiles.filter((file) => patterns.reduce((matched, pattern) => (0, pattern_1.isNegativeGlobPattern)(pattern)
            ? matched && !(0, minimatch_1.minimatch)(file, pattern.slice(1))
            : matched || (0, minimatch_1.minimatch)(file, pattern), false));
    }
    isCaseSensitive() {
        return true;
    }
    ...
    readDirSync(dirPath) {
        ...
        return directories
            .map((name) => ({
            name: (0, node_path_1.join)(dirPath, name),
            isFile: false,
            isDirectory: true,
            isSymlink: false,
        }))
    ...
    getAllFilePaths(path = '/', foundedFiles = []) {
        ...
        directories
            ...
            .forEach((dir) => {
            this.getAllFilePaths((0, node_path_1.join)(path, dir), foundedFiles);
        });
        return foundedFiles;
    }
}
