...
exports.migrateInputDateRange = migrateInputDateRange;
...
function migrateInputDateRange({ resource, recorder, fileSystem, }) {
    const template = (0, template_resource_1.getTemplateFromTemplateResource)(resource, fileSystem);
    const templateOffset = (0, template_resource_1.getTemplateOffset)(resource);
    const elements = (0, elements_1.findElementsByTagName)(template, 'tui-input-date-range');
    elements.forEach((element) => {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
        const sourceCodeLocation = element.sourceCodeLocation;
        (0, replace_tag_1.replaceTag)(recorder, sourceCodeLocation, 'tui-input-date-range', 'tui-textfield', template, templateOffset);
        ...
        for (const input of inputs) {
            input.attrs.forEach((attr) => {
                ...
                if (/^tuiTextfield$|^tuiTextfieldLegacy$/i.exec(attr.name)) {
                    ...
                    recorder.remove(templateOffset + startOffset, endOffset - startOffset);
                    recorder.insertRight(templateOffset + startOffset, `tuiInputDateRange${migrationAttrs}${placeholderAttr}`);
                }
            });
        }
    });
}
//# sourceMappingURL=migrate-input-date-range.js.map